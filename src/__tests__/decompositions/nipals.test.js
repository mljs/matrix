...
import simplePC15Loadings from '../../../data/simpleDatasetLoadings1-5.json';
import simplePC15 from '../../../data/simpleDatasetPC1-5.json';
...
describe('NIPALS pca', () => {
  it('test NIPALS dataArray to compare with R pca', () => {
    ...
    let corr2 = correlation(
      model2.t.clone(),
      Matrix.from1DArray(150, 1, irisPC14[1]),
    );
    ...
    let corr3 = correlation(
      model3.t.clone(),
      Matrix.from1DArray(150, 1, irisPC14[2]),
    );
    ...
    let corr4 = correlation(
      model4.t.clone(),
      Matrix.from1DArray(150, 1, irisPC14[3]),
    );
...
describe('NIPALS pls', () => {
  it('test NIPALS dataArray to compare with R pca', () => {
    ...
    let y = Matrix.from1DArray(150, 1, irisMetadata);
...
describe('NIPALS with simple data set', () => {
  it('test NIPALS error propagation', () => {
    ...
    let corr = correlation(
      model.t.clone(),
      Matrix.from1DArray(7, 1, simplePC15[0]),
    );
    expect(corr.get(0, 0)).toBeCloseTo(1, 6);
    expect(model.t.to1DArray().sort((a, b) => a - b)).toBeDeepCloseTo(
      simplePC15[0].sort((a, b) => a - b),
      3,
    );
    expect(model.w.to1DArray().sort((a, b) => a - b)).toBeDeepCloseTo(
      simplePC15Loadings[0].sort((a, b) => a - b),
      3,
    );
