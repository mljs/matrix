...
describe('multivariate linear regression', () => {
  it('correlation should work with 1 or 2 matrix inputs', () => {
    ...
    expect(Array.from(correlation(y).data[2])).toStrictEqual([
      -0.04272763026369126, -0.04272763026369126, 1, -0.04272763026369126,
      -0.04272763026369126,
    ]);
    expect(Array.from(correlation(y, x).data[2])).toStrictEqual([
      0.14685194996208847, 0.14685194996208847, 0.14685194996208847,
      0.14685194996208847, 0.14685194996208847,
    ]);
    expect(Array.from(correlation(x, y).data[2])).toStrictEqual([
      0.9819805060619657, 0.9819805060619657, 0.14685194996208847,
      0.9819805060619657, 0.9819805060619657,
    ]);
